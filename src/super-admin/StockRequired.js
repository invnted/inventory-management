...
function StockRequiredInStore() {
  ...
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(demands.length / itemsPerPage); i++) {
    pageNumbers.push(i);
  }
  ...
  return (
    <div>
      <div className='bg-white h-screen'>
        <Navbar />
        <div className='bg-sky-300 m-10 md:m-14 rounded-lg'>
            ...
            <div className="flex justify-center mt-4">
              {pageNumbers.map(number => (
                <button
                  ...
                  className={`mx-1 px-3 py-1 rounded-md ${currentPage === number ? 'bg-sky-900 text-white' : 'bg-gray-300 text-black'}`}
                >
                  {number}
                </button>
              ))}
